...
vst_count <- var_stabilize(spark_BC@counts) # R function in funcs.R

# the SE genes based on an FDR cutoff of 0.05
sig_vst_count <- vst_count[genes.osem7, ]
sig_vst_res <- t(apply(sig_vst_count, 1, LMReg, T = log(spark_BC@lib_size)))
...
position_cord <- spark_BC@location
