...
OrderShapeEM <- function (pvals, order.var,  control = OrderShapeEM.control()) {
	...
	# Global pi0 estimate
	pi0.est <- max(c(pi0est(pvals, lambda = seq(0.05, max(pvals), 0.05), pi0.method = 'bootstrap')$pi0,  
					pi0est(pvals,lambda = seq(0.05, max(pvals), 0.05), pi0.method = 'smoother')$pi0))
...
# 
# Installation path not writeable, unable to update packages: boot, class, cluster, codetools, foreign, KernSmooth, MASS,
# mgcv, nlme, nnet, spatial, survival
# Old packages: 'dplyr', 'gert', 'Matrix', 'utf8', 'vctrs'
# 'dplyr', 'gert', 'Matrix', 'utf8', 'vctrs'
